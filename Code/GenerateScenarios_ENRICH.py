# Here is the code for generating and modifying scenarios for ENRICH.
...
l = 1
for i in range(10,len(olddata.index),11):
    ranges[i-(10*l)][0] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 0"])
    ranges[i-(10*l)][1] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 1"])
    ranges[i-(10*l)][2] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 2"])
    ranges[i-(10*l)][3] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 3"])
    ranges[i-(10*l)][4] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 4"])
    ranges[i-(10*l)][5] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 5"])
    ranges[i-(10*l)][6] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 6"])
    ranges[i-(10*l)][7] = ast.literal_eval(olddata.loc[i, "Selected Bounds TIN 7"])
    l = l + 1

for j in range(len(ranges)):# given a range of input variables for one run of an algorithm (ENRICH), we generate 10 scenarios and modify
